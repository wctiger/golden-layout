...
let compileHooksIsRunning = false;
const data = { files: [], directories: [] };
const basePath = path.resolve(__dirname);
const jsPath = path.join(
  basePath,
  process.env.ES6 ? path.join('src', 'js_es6') : path.join('src', 'js')
);
const jsDir = path.join('dist', 'js');
const cssDir = path.join('dist', 'css');
const distFile = 'goldenlayout.js'; // for Prototype build only (ES6 is managed by webpack config)
...
module.exports = env => {
  ...
  return {
    ...
    entry: {
      ...
      [process.env.ES6 ? 'goldenlayout' : 'dummy']: env.dev
        ? `.${path.sep}app.js`
        : `.${path.sep}index.js`,
    },
    ...
    plugins: _.compact(
      [
        ...
        new CopyWebpackPlugin(
          process.env.ES6
            ? []
            : [{ from: path.join('..', 'lib', 'jquery.js'), to: `lib${path.sep}` }].concat(
                env.dev
                  ? [
                      { from: 'my_traces.js' },
                      {
                        from: path.join('..', 'lib', 'tracing.js', 'tracing.js'),
                        to: `lib${path.sep}`,
                      },
                    ]
                  : []
        ...
        process.env.ES6
          ...
          : new WebpackCompileHooksPlugin({
              onBuildStart() {
                ...
                const namespace = function(segments, node) {
                ...
                const walker = Walker(jsPath); // jshint ignore:line
                const newlineFile = path.sep + path.join('..', '..', 'build', 'newline.txt');
                const preWrapFile = path.sep + path.join('..', '..', 'build', 'wrapperStart.txt');
                const nsFile = path.sep + path.join('..', '..', 'build', 'ns.js');
                const postWrapFile = path.sep + path.join('..', '..', 'build', 'wrapperEnd.txt');
                ...
                walker.on('file', file => {
                  const filePath = file.replace(jsPath, '');
                ...
                walker.on('end', () => {
                  ...
                  fs.writeFileSync(
                    jsPath + nsFile,
                    `var lm=${JSON.stringify(data.directories)};\n`
                  );
                });
              },
              ...
              onBuildEnd() {
                console.log(chalk.cyan('\n>>> onBuildEnd\n'));
                const newlineFile = jsPath + data.files[0];
                const fileArray = _.flatten(_.map(data.files, p => [jsPath + p, newlineFile]));
                // console.log('final files:', fileArray)

                data.files = [];
                data.directories = [];
                compileHooksIsRunning = false;
                const prototypeDistFile = path.join(basePath, jsDir, distFile);
                concat(fileArray, prototypeDistFile, err => {
                  if (err) throw err;
                });
                env.build &&
                  setTimeout(() => {
        ...
        env.dev
          ? [
              new HtmlWebpackPlugin({
                ...
                support_library: process.env.JQUERY
                  ? path.join('lib', 'jquery.js')
                  : path.join('lib', 'zepto.js'),
              }),
